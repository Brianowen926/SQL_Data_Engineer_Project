--.read Lessons/1.22/1.22_DD_DML_pt2.sql

-- Tujuan kita ingin membuat 1 table dari gabungan jpf dan cd
-- DESCRIBE -- Untuk tahu semua nama kolom nya
-- SELECT *
-- FROM 
-- data_jobs.job_postings_fact AS jpf --Kali ini kita define nama DB nya
...
LEFT JOIN data_jobs.company_dim AS cd
ON jpf.company_id = cd.company_id;
--Kalo misal error, jalankan dulu .read 1.21 DD_DML_pt1.sql
...
WHERE
r.priority_lvl = 1; --cuman mau ngambil yang dta engineer
...
SELECT COUNT(*) FROM staging.job_postings_flat;
SELECT COUNT(*) FROM staging.priority_jobs_flat_view;
SELECT COUNT(*) FROM senior_jobs_flat_temp; --angka ini gak berubah karena jenis TEMP

--TRUNCATE akan menghapus semua isian table. Oleh karena itu kita masukan peinrta INSERT lagi
